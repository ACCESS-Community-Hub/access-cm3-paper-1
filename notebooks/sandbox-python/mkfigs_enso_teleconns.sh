...
#make the figures
array=( 16-DJF_PR_Teleconnection 17-DJF_ts_teleconnection 18-JJA_pr_teleconnection 19-JJA_ts_teleconnection )

for FNAME in "${array[@]}"
do
    echo "Running notebook: "${FNAME}".ipynb"
    python3 /home/548/cxc548/nesp/eval_metrics/ACCESS-eval/run_nb.py ${FNAME}.ipynb; papermill ${FNAME}.ipynb ${OFOL}${FNAME}_rendered.ipynb -p esm_file_pr ${ESMDIR_PR} -p esm_file_ts ${ESMDIR_TS} -p plotfolder ${OFOL} -p startyear ${STARTYR} -p endyear ${ENDYR} -p mylabel ${LABEL}; jupyter nbconvert --to markdown ${OFOL}${FNAME}_rendered.ipynb
done
