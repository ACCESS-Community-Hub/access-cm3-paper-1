...
def simpReg_slow (x, y):
    ...
    This function is slow for multi-dimensional arrays.
    """
...
def lagCorr1_or (x0, y0, nlag):
    ...
    This is slightly slower than crosscorr(), but same as lagCorr. 
    """
    import xarray as xr
    ...
    reg0 = simpRegCorr(x,y,corr=True)
    for lg in lagh:
        regP.append(simpRegCorr(x[:-lg],y[lg:],corr=True))
        regN.append(simpRegCorr(x[lg:],y[:-lg],corr=True))
...
def crossSpectrum(x, y, nperseg=256):
    # Not tested yet.
...
def cohSpectrum (x,y, nperseg=256):
    # Not tested yet.

    p11,freq=crossSpectrum(x,x,nperseg)
    p22,freq=crossSpectrum(y,y,nperseg)
    p12,freq=crossSpectrum(x,y,nperseg)
...
def butter_filter(data, cutfreq, fs, btype='low', order=5):
    """
    Low or high pass filtering using the Butterworth filter. Returns filtered data (y),
    Fourier frequencies (freq) and the filter response function (abs(h)).
    ...
    nyq = 0.5 * fs
    freq_cut = cutfreq / nyq
    b, a = butter(order, freq_cut, btype=btype)
...
def butter_bandpass_filter(data, lowcut, highcut, fs, order=5):
    """
    Bandpass filtering using the Butterworth filter. Returns filtered data (y),
    Fourier frequencies (freq) and the filter response function (abs(h)).
...
def wgt_areaave2 (indat, LatLon=None):
    lat=indat.lat
    lon=indat.lon
    if LatLon is None:
       latS = lat[0]
...
def mlr_df (df_target, df_forc, preds, dict_out=None):
    """
       Multiple linear regression analysis, with depenedent variables (df_target)
    and predictor variables (df_forc). The data must be pandas dataframe.
    Inputs:
       df_target: depenedent variables
       df_forc  : predictor variables
       preds    : predictor names (must be a list)

    outputs:
       A list of many computed stats

    The calculation of "Cov" is adhoc and may not be accurate for higly correlated predictors.

    %timeit out_list3 = ms.mlr_df (df_tas0.loc[syr1:syr2,target], df_forc2, preds)
     30.9 s ± 102 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
    """
    import statsmodels.formula.api as sm

    #if type(df_target) != pd.DataFrame or type(df_forc) != pd.DataFrame:
    if type(df_target) != pd.DataFrame:
        print("First input must be a pandas dataframes.")
        return

    if type(df_forc) != pd.DataFrame:
        if len(df_forc.shape) == 1 and len(preds) == 1:
            df_forc = pd.DataFrame(df_forc,index=df_target.index,columns=preds)
        elif len(df_forc.shape) == 2 and df_forc.shape[1] == len(preds):
            df_forc = pd.DataFrame(df_forc,index=df_target.index,columns=preds)
        else:
            print("Second input must be a pandas dataframes.")
            return
    
    target = df_target.columns
    pred2 = ' + '.join(preds)
    predx = preds+['Resid']

    df_vars = pd.DataFrame(index=preds+['Cov','Resid'], columns=target)
    df_varL = pd.DataFrame(index=preds, columns=target)
    df_varU = pd.DataFrame(index=preds, columns=target)
    df_pval = pd.DataFrame(index=['Intercept']+preds, columns=target)
    df_params = df_pval.copy()
    df_CI5  = df_pval.copy()
    df_CI95 = df_pval.copy()
    df_rsquared = pd.Series(index=target,dtype="float64")
    df_pred = pd.DataFrame(index=df_target.index, columns=target)
    df_resid = pd.DataFrame(index=df_target.index, columns=target)

    temps = {}
    for i,t in enumerate(target):
        if len(preds) == df_forc.shape[1]:
            df_data = pd.concat([df_target[t],df_forc],axis=1)
        else:
            t2 = [p+'_'+t for p in preds] 
            df_data = pd.concat([df_target[t],df_forc[t2]],axis=1)
            df_data.columns = [t]+preds
        result = sm.ols("df_data[t] ~ "+pred2, df_data).fit()
        dset = target[i]
        params = result.params
        temp = params[preds]*df_data[preds]
        temp['Intercept'] = params['Intercept']
        temps[dset] = temp
        df_params[dset] = params
        conf_ints = result.conf_int(alpha=0.05,cols=None)  # 5-95% confidence interval using t-test
        df_CI5[dset] = conf_ints[0]                        # 5% CI
        df_CI95[dset] = conf_ints[1]                       # 95% CI
        tempL = conf_ints.loc[preds,0]*df_data[preds]
        tempU = conf_ints.loc[preds,1]*df_data[preds]
        df_pval[dset] = result.pvalues                     # p-values
        df_rsquared[dset] = 100*result.rsquared_adj        # expVars
        df_pred[dset] = result.fittedvalues            # => temp.sum(axis=1) + result.params[0]
        df_resid[dset] = result.resid                   # same as below
        tmpv = temp.var()
        tmpv['Resid'] = df_resid[dset].var()
        df_vars[dset] = 100*tmpv/df_target[dset].var()
        df_vars.loc['Cov'] = 100 - df_vars.loc[predx,:].sum(axis=0)
        df_varL[dset] = 100*tempL.var()/df_target[dset].var()
        df_varU[dset] = 100*tempU.var()/df_target[dset].var()

    out_list = [df_vars,df_varL,df_varU,df_rsquared,df_pval,df_CI5,df_CI95,df_params,df_pred,df_resid,temps]
    if dict_out:
        keys = ['vars','varL','varU','rsquared','pval','CI5','CI95','params','pred','resid','temps']
        return dict(zip(keys,out_list))
    else:
        return out_list

def mlr_df2 (df_target, dict_forc0, preds, dict_out=None):
    """
       Multiple linear regression analysis, with depenedent variables (df_target)
    and predictor variables (df_forc). The data must be pandas dataframe.
    Inputs:
       df_target: depenedent variables, a 1-d or 2-d DataFrame
       dict_forc: a dictionary of predictor variables (Series and/or DataFrames)
       preds    : predictor names (must be a list)

    outputs:
       A list of many computed stats

    This is a second version of mlr_df, with the 2-d predictor dataframe built inside this
    function, making the main script less complicated. For mlr_df, the 2-d predictor dataframe 
    has to be built in the calling script.

    The executation speeds for both functions are yet to be tested. 

    The calculation of "Cov" is adhoc and may not be accurate for higly correlated predictors.
    """
    import statsmodels.formula.api as sm

    #preds = list(dict_forc.keys())
    if list(dict_forc0.keys()) == preds:
        dict_forc = dict_forc0
    else:
        dict_forc = {k:dict_forc0[k] for k in preds}
    ...
    df_forc1 = pd.DataFrame({k:dict_forc[k] for k in preds if len(dict_forc[k].shape)==1})
    preds_2d = [k for k in preds if len(dict_forc[k].shape)==2]

    if preds_2d:
        target = dict_forc[preds_2d[0]].columns
    ...
    for i,t in enumerate(target):
        if preds_2d:
            df_forc2 = pd.DataFrame({k:dict_forc[k][t] for k in preds_2d})
            df_forc = pd.concat([df_forc1,df_forc2],axis=1)
        else:
            df_forc = df_forc1
        #if len(preds) == df_forc.shape[1]:
        df_data = pd.concat([df_target[t],df_forc],axis=1)
...
def mlrDT_df (df_target, df_forc, pred, nlag=1, dict_out=None):
    """
       Multiple linear regression analysis, with depenedent variables (df_target)
    and predictor variables (df_forc). The data must be pandas dataframe.
    Inputs:
    ...
    This version also includes the time derivative of df_target (tas) as an independent variable.
    """
    import statsmodels.formula.api as sm
    ...
    temps = {}
    for i,t in enumerate(target):
        if len(pred) == len(df_forc.columns):
            df_data = pd.concat([df_target[t],df_target[t].diff(nlag),df_forc],axis=1).dropna(axis=0)
        else:
            t2 = [p+'_'+t for p in pred] 
            df_data = pd.concat([df_target[t],df_target[t].diff(nlag),df_forc[t2]],axis=1).dropna(axis=0)
        df_data.columns = [t]+preds
        result = sm.ols("df_data[t] ~ "+pred2, df_data).fit()
        dset = target[i]
        params = result.params
...
def mlrLag_df (df_target, df_forc, pred, nlag=1, dict_out=None):
    """
       Multiple linear regression analysis, with depenedent variables (df_target)
    and predictor variables (df_forc). The data must be pandas dataframe.
    Inputs:
    ...
    This version also includes a lagged df_target (tas) as an independent variable.
    Note: The interaction term ("Cov") in this case is high, as lagged_tas is highly
    ...
    for i,t in enumerate(target):
        if len(pred) == len(df_forc.columns):
            df_data = pd.concat([df_target[t],df_target[t].shift(nlag),df_forc],axis=1).dropna(axis=0)
        else:
            t2 = [p+'_'+t for p in pred] 
            df_data = pd.concat([df_target[t],df_target[t].shift(nlag),df_forc[t2]],axis=1).dropna(axis=0)
        df_data.columns = [t]+preds
        result = sm.ols("df_data[t] ~ "+pred2, df_data).fit()
        dset = target[i]
        params = result.params
...
def mlr_sr (df_target, df_forc, preds=None):
    """
       Multiple linear regression analysis, with depenedent variables (df_target)
    and predictor variables (df_forc). The data must be pandas dataframe.
    Inputs:
       df_target: pd.Series, depenedent variables
       df_forc  : pd.DataFrame, predictor variables
       preds    : predictor names (must be a list)

    outputs:
       result: An object containing MLR results

    %timeit =>> 3.22 ms ± 15.2 µs per loop.
    This function is a bit slower than 'mlr' below, for which:
       %timeit =>> 787 µs ± 10.1 µs per loop 
    as 'mlr' does numpy-level calculations. 
    """
    import statsmodels.formula.api as sm


    if preds is None:
        preds = df_forc.columns
    else:
        df_forc = df_forc[preds]
    pred2 = ' + '.join(preds)
    df_data = pd.concat([df_target,df_forc],axis=1)    
    result = sm.ols("df_data.iloc[:,0] ~ "+pred2, df_data).fit()
    return result

def mlr (y, x, intercept=True, alpha=0.05):
    """
    A faster multiple linear regression function (May be verified against mlr_sr above).
    (From https://currents.soest.hawaii.edu/ocn_data_analysis/_static/regression.html).
    
        y: 1-D or 2-D array, dependent variable.
            (ntimes,) or (ntimes, nrealizations)
        x: 2-D array, model; (ntimes, nparams), independent variable
    ...
    if intercept:
        ones = np.ones(len(y))
        x = np.concatenate((ones[:,np.newaxis],x),axis=1) # add 1s for intercept

    ntimes, nparams = x.shape
    if y.shape[0] != ntimes:
        raise ValueError("Shape mismatch: x is %s, y is %s" %
                         x.shape, y.shape)
    pinv = scipy.linalg.pinv(x)
    b = np.dot(pinv, y)
    fit = np.dot(x, b)
    residuals = y - fit
    
    eps_std = np.std(residuals, axis=0, ddof=nparams, keepdims=True)
    inv_vcv = np.linalg.inv(np.dot(x.T, x))
    b_std = (np.sqrt(np.diag(inv_vcv)) * eps_std.T).T
    t_stat = b/b_std
    #p_val = pval_stdErr(b, b_std, ntimes, nparams)
    b_t = scipy.stats.t(ntimes - nparams, loc=b, scale=b_std)
    b_lo = b_t.ppf(alpha/2)
    b_hi = b_t.ppf(1 - alpha/2)
    #print(b_t)
    p_val = scipy.stats.t.sf(np.abs(t_stat), ntimes - nparams)
    out = dict(x=x, y=y, b=b, p_val=p_val, b_std=b_std, tstat=t_stat, b_lo=b_lo,
...
def mlr_gappy(y, x, alpha=0.05):
    ...
    out = dict(b=b, b_lo=b_lo, b_hi=b_hi, b_std=b_std, 
...
def bootstrap_mlr5(y,x3,ncopy=10000,method='redn',ci=[0.05,0.95]):
    ''' Calculates bootstrap confidence intervals for residuals of MLR5 model.
    Inputs:
        y => input (dependent variable) time series (1-D)
        x3 => input (independent variable) time series (2-D) => [:,3]
        ncopy => number of bootstrap samples (default=1000, minimum number)
        method => method of generating synthetic timeseires: 'boot' or 'redn' [default]
        ci => desired confidence levels (default = 5%,95%, one tailed for res variances)
    Returns:
        resVar0 => residual_variance of parent copy from MLR5 model
        conf_interval => confidence interval values (one-tailed)
        resVars => sorted residual variances for all bootstrap samples (ncopy)
        out_dict0 => full MLR results for the parent copy

    Explore block bootstrap: 
    (https://stackoverflow.com/questions/51641223/block-bootstrapped-sampling-in-pandas)
    (https://www.sktime.net/en/stable/api_reference/auto_generated/sktime.transformations.bootstrap.MovingBlockBootstrapTransformer.html)

      bootstrapped_df = (pd.DataFrame({'personid':np.random.choice( personids, size=personids.size, 
                                                             replace=True)}).merge(df,how='left'))
    '''
    ...
    ci2 = int(ci[1]*ncopy)
    ones = np.ones(len(y))
    xt2 = np.concatenate((ones[:,np.newaxis],xt),axis=1) # add 1s for intercept
    out_dict0 = mlr(y, xt2, intercept=False, alpha=0.05)
    resVar0 = 100*out_dict0['residuals'].var()/yvar
    if method == 'boot':                                 # bootstrap method  (slightly faster, 7.97 s
        idx = np.random.randint(len(x),size=(ncopy,len(x))) # ncopy=10000)
        bx = x[idx]
    elif method == 'redn':                                  # red-noise copies (slightly slower, 8.44 s)
        bx = red_noise(x,ncopy).T
    resVars = []
    for i in range(ncopy):
        xt3 = xt2.copy()
        xt3[:,3] = bx[i,:]
        out_dict = mlr(y, xt3, intercept=False, alpha=0.05)
        resVars.append(100*out_dict['residuals'].var()/yvar)

    resVars.sort()
    conf_interval = (resVars[ci1],resVars[ci2])

    return resVar0, conf_interval, resVars, out_dict0
...
def get_tsDAMIP (dict_tasd,key):
    """Calculate the separate ensemble-mean timeseries for each model and ext. forcing
       from the DAMIP ensemble. Specifically, get a multi-model, multi-ensemble member
       dataframe of DAMIP data from dict_tasd, and:

          1) Calculate the ensemble means for each model
          2) Separate the GHG, aer, and nat forcing components
          3) Return the list of three DataFrames

    Inputs:
       dict_tasd => a dict of data (see comp_CMIP6-DAMIP_tas_v2.py)
       key => key
    """

    df_tasd0 = dict_tasd[key]
    modNames = set()
    models = df_tasd0.columns
    for m in models:
        modNames.add(m.split('_')[0])

    modNames = list(modNames)
    modNames.sort()
    dict_temp = dict()
    df_tasd = pd.DataFrame()
    for m in modNames:
        for c in ['GHG','aer','nat']:
            dict_temp[m+'_'+c] = df_tasd0[[v for v in models if m+'_'+c+'_' in v]]
            df_tasd[m+'_'+c] = dict_temp[m+'_'+c].mean(axis=1)

    cols = df_tasd.columns
    col_ghg = [c for c in cols if 'GHG' in c]
    col_aer = [c for c in cols if 'aer' in c]
    col_nat = [c for c in cols if 'nat' in c]

    df_tasd_ghg = df_tasd[col_ghg]
    df_tasd_aer = df_tasd[col_aer]
    df_tasd_nat = df_tasd[col_nat]
    df_tasd_ghg.columns = modNames
    df_tasd_aer.columns = modNames
    df_tasd_nat.columns = modNames

    return [df_tasd_ghg,df_tasd_aer,df_tasd_nat]
...
def comp_Stats1 (Ridx, Rstd=None, Rposneg="pos", time=None):
    ...
    Inputs:
        ...
        Rposneg => Stats for ositive/negative phase
...
def comp_multEvents (ts, Rstd=None, nsmooth=3, window='flat', time=None):
    """Computes stats for consequetive El Nino or La Nina events from an ENSO index.

    Input:
       ts => An anomalous timeseries (e.g., the Nino3.4 index)
       Rstd => Threshold value for defining +ve or -ve events (default: 1 std)
       nsmooth => Integer value for window length
       window => smoothing window ['flat','hanning', 'hamming', 'bartlett' or 'blackman']
       time => DataFrame index or xarray time coordinate
    ...
    Outputs:
       df_sorted => A sorted and concatenated dataframe of positive and negative ENSO
                    stats
       gb => A groupby object for indentifying multiple consequetive pos/neg events

    Usage:
       df4_sorted, gb = ms.comp_multEvents (ts)
       cons_PosNeg_events = gb.count()  # Numbers of conscutive El Nino or La Nina events

       for k,v in gb.groups.items():
           if cons_PosNeg_events[k] > 1:
               print('Group:'+str(k),' Count:'+str(cons_PosNeg_events[k]))
               print(df4_sorted.loc[v,['Durations','MaxMinval','posneg','StartDate','EndDate']])

       labels, counts = np.unique(cons_PosNeg_events, return_counts=True)
       plt.bar(labels, counts, align='center')
       plt.show()

    See, e.g., $wd/Eval_CMIP6/CM2_ensMems/enso_work/comp_MultipleEvents.py

    14-NOV-2023  Harun Rashid    
    """

    # Compute ENSO event stats for both phases
    df_stats_pos,df_stats_neg = comp_Stats (ts,Rstd=Rstd,nsmooth=nsmooth,window=window,time=time)

    # Find the occurrences of consecutive El Nino and La Nina evenst

    df4 = pd.concat([df_stats_pos,df_stats_neg],axis=0,ignore_index=True)
    df4_sorted = df4.sort_values('MaxMinInd')
    bull_arr = np.equal(df4_sorted['posneg'],'pos')
    df4_sorted['grouping_label'] = (bull_arr != bull_arr.shift(1)).cumsum()
    gb = df4_sorted['grouping_label'].groupby(df4_sorted['grouping_label'])

    return df4_sorted, gb
...
def compIndexF0 (tas):
    """Calculate spatially-averaged climate indices from SAT field. Only 3-d fields
    (time,lat,lon) are accepted now.
    ...
    #xr_inds = xr.concat(list_inds,dim='indx').T
    nregs = len(dict_coords.keys())
    xr_inds = xr.DataArray(list_inds,dims=('indx','time'),coords={'indx':np.arange(nregs),'time':tas.time})
    xr_inds.attrs['name'] = 'clim_inds'
    if hasattr(tas,'units'): xr_inds.attrs['units'] = tas.units
    if hasattr(tas,'long_name'): xr_inds.attrs['long_name'] = "Spatially averaged climate indices of "+tas.long_name
    xr_inds.coords['indx'] = list(dict_coords.keys())
    xr_inds.coords['indx'].attrs['long_name'] = "Climate indices"

    #df=xr_inds.to_pandas()
    return xr_inds

def compIndexF_cdo (inFile, indx=None):
    """Calculate spatially-averaged climate indices from SAT field. The last two dims 
    are assumed (lat,lon). Selected indices may be calculated, as opposed to all. This
    CDO version gives similar results as compIndexF* above, this is 6 times slower than
    compIndexF. Use it for checking results from compIndexF only.

    The CDO package and its python binding must be installed for this function to work.

    Input:
       inFile => Input filename of a 3-d SAT or any other fields.
    Output:
       xr_inds => An xarray DataArray with (ntime,nindex) dimensions.
    ...
    if 'IOD' in regNames:
        for reg in ['IODW','IODE']:
            v = dict_coords[reg]
            sarea = str(v[2])+','+str(v[3])+','+str(v[0])+','+str(v[1]) 
            temp = cdo.fldmean(input=f'-sellonlatbox,{sarea} {inFile}', returnXDataset=True)
            dict_temp[reg] = temp.to_array().squeeze()
            #    dict_temp[reg] = wgt_areaave(tas.sel(lat=slice(v[0],v[1]),lon=slice(v[2],v[3])))
    if 'TPI' in regNames:
        for reg in ['TPI-N','TPI-T','TPI-S']:
            v = dict_coords[reg]
    ...
    for reg in regNames:
        if reg == 'IOD':
            ...
            temp = cdo.fldmean(input=f'{inFile}', returnXDataset=True)
            list_inds.append(temp.to_array().squeeze())
        else:
            v = dict_coords[reg]
            sarea = str(v[2])+','+str(v[3])+','+str(v[0])+','+str(v[1]) 
            temp = cdo.fldmean(input=f'-sellonlatbox,{sarea} {inFile}', returnXDataset=True)
...
def read_mtdData(inFile,expt='hist',ivar='mtd'):
    """Calculate spatially-averaged climate indices from thermocline depth field. Only 2-d fields
    (time,lon) are accepted now.

    Input:
       inFile => Input MTD file name
       expt   => Experiment name ['historical', 'ssp126', 'ssp245', 'ssp370', 'ssp585']
    Output:
       A dictionary of DataFrames of Pacific indices ['EqPac', 'Nino3.4', 'Nino3', 'Nino4', 'WarmPool']
    ...
    'Nino3': [210.0, 270.0],
    'Nino4': [160.0, 210.0],
    'WarmPool': [160.0, 200.0]
    }
    if 'hist' in expt:
        dates = pd.date_range('1850-01-01',periods=1980,freq='MS')
    elif 'ssp' in expt:
        dates = pd.date_range('2015-01-01',periods=12*(2100-2015+1), freq='MS')
    elif 'piCont' in expt:
        dates = None
    ...
    for k,v in f.variables.items():
        if len(v.shape) == 2 and ivar in k:
            m = '_'.join(k.split('_')[1:])
            temp = f[k]
            dict_temp = {}
            for i,c in dict_coords.items():
                dict_temp[i] = temp.sel(lon=slice(c[0],c[1])).mean('lon').values
            df_temp = pd.DataFrame(dict_temp)
            if dates is not None:
                df_temp = df_temp.iloc[:len(dates)]  # discard data beyond 2100 for some expts/members
                df_temp.index = dates
            dict_data[m] = df_temp
   
    return dict_swap(dict_data)
...
def diag_table_ens (dict_data, ufld='UAS4', K2C=273.15, exMods=[]):
    """ Calculates ENSO related metrics from input Nino sea surface temperatures (SSTs), 
    zonal wind stress (TAUU), and thermoclice depth (THCD) data. This function is for
    single realisations only (e.g., obs or individual ensemble simulations). For multiple
    ensemble simulations, use diag_table or diag_table_df.

    NB: The columns names are assumed to be: 'SST3','TAUU','THCD','SST4' (in any order).
    Inputs:
        dict_data - Dictionary containing the following dictionaries with keys:
           SST3 - Nino-3 SST data (full data, not anomalies)
           SST4 - Nino-4 SST data
           TAUU - Nino-4 zonal wind stress data, or
           UAS4 - Nino-4 surface zonal winds
           THCD - Equatorial Pacific (-0.5,0.5; 124-277) averaged thermoclice depth data
           K2C  - SST value to be subtracted for Kelvin to Celcius conversion (0, if SST is in C)
           exMods - Exclude models with incomplete data

    This and the below functions are for dictionaries of DataFrames. This is ~ 3 times slower
    than diag_table below, but is more structured (hence, easier to maintain).

    ---
    Harun Rashid
    09-SEP-2024
    05-MAY-2025 (added "if k in exMods: continue"; renamed: diag_table_df->diag_table_ens)
    """

    if isinstance(dict_data['SST3'],dict):  # dict_data is a dictionary of dictionaries
        dict_ts_nino3 = dict_data['SST3']
        dict_ts_nino34 = dict_data['SST34']
        dict_ts_nino4 = dict_data['SST4']
        dict_tauu4 = dict_data[ufld] 
        if ufld[:-1]+'34' in dict_data.keys():
            dict_tauu34 = dict_data[ufld[:-1]+'34']
        dict_mtd = dict_data['THCD']
        expts = list(dict_ts_nino3.keys())
        dict_diag = {}
        for expt in expts:
            df_diag = {}
            for col in dict_ts_nino3[expt].columns:
                if ufld[:-1]+'34' in dict_data.keys():
                    df_data = pd.concat([dict_ts_nino3[expt][col],dict_ts_nino34[expt][col],dict_ts_nino4[expt][col], dict_mtd[expt][col],dict_tauu4[expt][col],dict_tauu34[expt][col]],axis=1)
                    df_data.columns = ['SST3','SST34','SST4','THCD',ufld,ufld[:-1]+'34']
                else:
                    df_data = pd.concat([dict_ts_nino3[expt][col],dict_ts_nino34[expt][col],dict_ts_nino4[expt][col], dict_mtd[expt][col],dict_tauu4[expt][col]],axis=1)
                    df_data.columns = ['SST3','SST34','SST4','THCD',ufld]
                df_diag[col] = diag_table_df (df_data,ufld=ufld)
                #df_diag[col]['SST3mean'] = df_diag[col]['SST3mean'] - 273.15
                #df_diag[col]['SST34mean'] = df_diag[col]['SST34mean'] - 273.15
                #df_diag[col]['SST4mean'] = df_diag[col]['SST4mean'] - 273.15
            df_temp = pd.DataFrame(df_diag).T
            df_temp[[c for c in df_temp.columns if 'SST' in c and 'mean' in c]] -=  K2C
            dict_diag[expt] = df_temp
        return pd.concat([v for v in dict_diag.values()])
    elif isinstance(dict_data['SST3'],pd.DataFrame): # # dict_data is a dictionary of DataFrames
        dict_datap = dict_swap(dict_data)
        df_diag = {}
        for k,v in dict_datap.items():
            if k in exMods: continue
            df_diag[k] = diag_table_df (v,ufld=ufld)
        df_diag = pd.DataFrame(df_diag).T
        df_diag[[c for c in df_diag.columns if 'SST' in c and 'mean' in c]] -= K2C       
        return df_diag
    else:
        print('Input must be a dict() of dicts or dict() of DataFrames!')
        return

def diag_table (dict_data, ufld='UAS4', K2C=273.15):
    """ Calculates ENSO related metrics from input Nino sea surface temperatures (SSTs), 
    zonal wind stress (TAUU), and thermoclice depth (THCD) data. This function is for
    single realisations only (e.g., obs or individual ensemble simulations). For multiple
    ensemble simulations, use diag_table or diag_table_df.

    NB: The columns names are assumed to be: 'SST3','TAUU4/UAS4','THCD','SST4' (in any order).
    Inputs:
        dict_data - Dictionary containing the following dictionaries with keys:
           SST3 - Nino-3 SST data (full data, not anomalies)
           SST4 - Nino-4 SST data
           TAUU4 - Nino-4 zonal wind stress data, or
           UAS4 - Nino-4 surface zonal winds
           THCD - Equatorial Pacific (-0.5,0.5; 124-277) averaged thermoclice depth data
           K2C  - SST value to be subtracted for Kelvin to Celcius conversion (0, if SST is in C)

    This and the above functions are for dictionaries of DataFrames. This is ~ 3 times faster
    than diag_table_ens, but the latter is more user friendly.
    ---
    Harun Rashid
    09-SEP-2024
    """
    ...
    dict_ts_nino34 = dict_data['SST34']
    dict_ts_nino4 = dict_data['SST4']
    dict_tauu = dict_data[ufld] 
    dict_mtd = dict_data['THCD']

    if isinstance(dict_ts_nino3,dict):
        expts = list(dict_ts_nino3.keys())
    else:
        print('Input dictionary must contain other dictionary(ies); e.g., expts')
        return
    ...
    df_timeScale_spec = {}
    max_lag_corr_mtd_ts = {}
    max_lag_corr_ts_tauu = {}
    max_lag_corr_tauu_ts = {}
    df_corr_mtd_ts = []
    df_corr_ts_tauu = []
    df_corr_tauu_ts = []
    df_enso_stats = []
    for expt in expts:
        df_ts_tmn3[expt] = dict_ts_nino3[expt].mean()
        df_ts_tmn34[expt] = dict_ts_nino34[expt].mean()
        df_ts_tmn4[expt] = dict_ts_nino4[expt].mean()
        df_ts_diff[expt] = dict_ts_nino4[expt].mean() - dict_ts_nino3[expt].mean() # -ve of SST gradient
        df_tauu_tmn[expt] = dict_tauu[expt].mean()
        df_mtd_tmn[expt] = dict_mtd[expt].mean()
        dict_ts_ano[expt] = anomaly_df(detrend_df(dict_ts_nino3[expt],2))
        dict_tauu_ano[expt] = anomaly_df(detrend_df(dict_tauu[expt],2))
        dict_mtd_ano[expt] = anomaly_df(detrend_df(dict_mtd[expt],2))
        dict_data_ano_stdac[expt] = dict_ts_ano[expt].groupby(dict_ts_ano[expt].index.month).std()
        df_ts_std[expt] = dict_ts_ano[expt].std()
        df_tauu_std[expt] = dict_tauu_ano[expt].std()
        df_mtd_std[expt] = dict_mtd_ano[expt].std()
        df_ts_skew[expt] = dict_ts_ano[expt].skew()
        #dict_ts_ano[expt] /= df_ts_std[expt] 
        #dict_tauu_ano[expt] /= df_tauu_std[expt]
        #dict_mtd_ano[expt] /= df_mtd_std[expt]
        df_temp = dict_ts_ano[expt].apply(spectrum, args=(19,'hanning'))
        df_temp.index = df_temp.index*12        # cyc/mon to cyc/year
        dict_ts_spec[expt] = df_temp
        df_peakPeriod_spec[expt] =  1./dict_ts_spec[expt].idxmax()  # Period of spectral peak
        df_spc1 = dict_ts_spec[expt].loc[:1.0]   # Corresponds to ~2.5-yr spectral peak for ACCESS-CM2
        df_SpecFreq = df_spc1.T.dot(df_spc1.index)/df_spc1.sum()  # sum(spec*freq)/sum(spec); same as below
        #df_SpecFreq = (df_spc1.values*df_spc1.index.values[:,np.newaxis]).sum(axis=0)/df_spc1.sum(axis=0) 
        df_timeScale_spec [expt] = 1/df_SpecFreq        # Eq on page 6297 of Branstator and Teng (2010), JC.
        # Only +ve lags are relevant for the feedbacks (see below)
        df_corr = lagReg(dict_mtd_ano[expt],dict_ts_ano[expt],20) # unit: degC
        df_corr_mtd_ts.append(df_corr)
        max_lag = df_corr.idxmax()   # Only +ve lags are relevant for the forcing/feedbacks (hence, idxmax())
        max_corr = [df_corr.loc[max_lag.iloc[i],c] for i,c in enumerate(df_corr.columns)]
        max_lag_corr_mtd_ts[expt] = pd.DataFrame({'maxCorr_tcd':max_corr,'maxLag':max_lag.values},index=max_lag.index)

        df_corr = lagReg(dict_ts_ano[expt],dict_tauu_ano[expt],10) # unit: N/m^2
        df_corr_ts_tauu.append(df_corr)
        max_lag = df_corr.idxmax()
        #max_corr = [df_corr.loc[max_lag.iloc[i],c] for i,c in enumerate(df_corr.columns)]
        sim_corr = df_corr.T[0]
        max_lag_corr_ts_tauu[expt] = pd.DataFrame({'maxCorr_tauu':sim_corr,'maxLag':max_lag.values},index=max_lag.index)

        df_corr = lagReg(dict_tauu_ano[expt],dict_ts_ano[expt],10) # unit: degC
        df_corr_tauu_ts.append(df_corr)
        max_lag = df_corr.idxmax()
        max_corr = [df_corr.loc[max_lag.iloc[i],c] for i,c in enumerate(df_corr.columns)]
        max_lag_corr_tauu_ts[expt] = pd.DataFrame({'maxCorr_tauu':max_corr,'maxLag':max_lag.values},index=max_lag.index)
        df_temp = {}
        for col in dict_ts_ano[expt].columns:
            df_pos,df_neg = comp_Stats(dict_ts_ano[expt][col].squeeze())
            df_temp[col] = [df_pos['Durations'].mean(),df_neg['Durations'].mean(),
                            df_pos['MaxMinval'].mean(),df_neg['MaxMinval'].mean()]
        df_temp = pd.DataFrame(df_temp,index=['EN_Duration','LN_Duration','EN_MaxVal','LN_MinVal']).T
        df_enso_stats.append(df_temp)

    df_enso_stats = pd.concat(df_enso_stats)
    max_lag_corr_mtd_ts = dict_swap(max_lag_corr_mtd_ts)
    max_lag_corr_ts_tauu = dict_swap(max_lag_corr_ts_tauu)
    max_lag_corr_tauu_ts = dict_swap(max_lag_corr_tauu_ts)

    df_metrics = pd.DataFrame()
    df_metrics['SST3mean'] = pd.DataFrame(df_ts_tmn3).sum(axis=1) - K2C
    df_metrics['SST34mean'] = pd.DataFrame(df_ts_tmn34).sum(axis=1) - K2C
    df_metrics['SST4mean'] = pd.DataFrame(df_ts_tmn4).sum(axis=1) - K2C
    df_metrics['SSTdiff'] = pd.DataFrame(df_ts_diff).sum(axis=1)      # -ve of SST gradient
    df_metrics[ufld+'mean'] = pd.DataFrame(df_tauu_tmn).sum(axis=1)
    df_metrics['TCDmean'] = pd.DataFrame(df_mtd_tmn).sum(axis=1)
    df_metrics['SST3stdv'] = pd.DataFrame(df_ts_std).sum(axis=1)
    df_metrics[ufld+'stdv'] = pd.DataFrame(df_tauu_std).sum(axis=1)
    df_metrics['TCDstdv'] = pd.DataFrame(df_mtd_std).sum(axis=1)
    df_metrics['SST3skew'] = pd.DataFrame(df_ts_skew).sum(axis=1)
    df_metrics['PhaseLockRange'] = pd.concat([dict_data_ano_stdac[expt].max()-dict_data_ano_stdac[expt].min() for expt in expts],axis=0)
    df_metrics['PhaseLockMaxMon'] = pd.concat([dict_data_ano_stdac[expt].idxmax() for expt in expts],axis=0)
    df_metrics['PhaseLockMinMon'] = pd.concat([dict_data_ano_stdac[expt].idxmin() for expt in expts],axis=0)
    df_metrics['ENSOpeakPeriod'] = pd.DataFrame(df_peakPeriod_spec).sum(axis=1)
    df_metrics['ENSOtimeScale'] = pd.DataFrame(df_timeScale_spec).sum(axis=1)
    df_metrics['ThermoclineFdbk'] = max_lag_corr_mtd_ts['maxCorr_tcd'].sum(axis=1)
    df_metrics['maxLag_TCFdbk'] = max_lag_corr_mtd_ts['maxLag'].sum(axis=1)
    df_metrics['BjerknessFdbk'] = max_lag_corr_ts_tauu['maxCorr_tauu'].sum(axis=1)
    df_metrics['maxLag_BJFdbk'] = max_lag_corr_ts_tauu['maxLag'].sum(axis=1)
    df_metrics['ZonalWindForc'] = max_lag_corr_tauu_ts['maxCorr_tauu'].sum(axis=1)
    df_metrics['maxLag_ZWForc'] = max_lag_corr_tauu_ts['maxLag'].sum(axis=1)
    df_metrics['EN_Duration'] = df_enso_stats['EN_Duration']
    df_metrics['LN_Duration'] = df_enso_stats['LN_Duration']
    df_metrics['EN_MaxVal'] = df_enso_stats['EN_MaxVal']
    df_metrics['LN_MinVal'] = df_enso_stats['LN_MinVal']

    return df_metrics.astype('float')
